//! Emission of the resource family (see `entities/resource.rs`).
...
impl<'k, K: Knowledge> Axiom<'k, K> {
    pub fn emit_resource(&self, input: ResourceInput) -> Result<Resource, AxiomError> {
        Ok(Resource::create(input)?)
    }

    pub fn emit_resource_instance(
        &self,
