...
| named in | item |
|---|---|
| 05 06 07 08 09 12 15 | **Saying two lines agree** — measured three times, expressible nowhere, and now **shaped**. 12 found that a *carried* decision cannot be written down; 15 found a third move it had not considered — do not carry the decision, **retake** it — and measured what that buys. What the item wanted was an operation; what it has is that the operation is **composition** over the public surface, that the useful half of it is about **knowledge** and not about intention, and that the other half cannot be made useful: a retaken world is never a world the other record decided, so *take all of their decisions* is the only answer there is. Still open, and what remains open is narrower than it was |
| 15 | **A merge that says what was takeable** — the one **defect** 15 found rather than an absence. `converge` answers `Diverged { position }`, and a caller reading it concludes the two records are incompatible; measured, ten of the other record's entries and both of its decisions could have been taken. It belongs with 13's Request 1 — refusals that name the wrong half of what a reader needs — and the two should be answered together or not at all |
