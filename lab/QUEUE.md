...
| named in | item |
|---|---|
| 15 | **A fork is portable and a genesis is translatable** — of the two shapes an intention takes, the one carrying no instant crosses unchanged and the one carrying an instant crosses only by having it replaced. Not a request yet; it is the fact anything asking about intention between records now starts from, and it is here so the next protocol does not rediscover it |
| 13 | **An outcome for *I cannot answer for this decision*** — distinct from refusing, and still the same value. A record holding nothing refuses a true decision exactly as it refuses a false one, so a reader cannot tell *not mine* from *corrupt*. **13's Request 1 has been repaired and this was not**, which is the useful thing now known about it: the repair made the refusal say *the decision was taken against another journal*, and 13's audit column stayed a constant. A reader has stopped being sent to look for a corrupted address and has not gained an outcome. The two requests looked adjacent and are one apart — this one asks for something that is not a refusal |
...
| 16 | **A refusal that says which side is missing, in the engine** — the half of 13's request that survives its repair, and it is not the application's to make. A record that lost an entry no world was a function of is refused, later, by a fork it can no longer take, and told `UnknownCommitment` — true, and not the reason. That refusal is `ThesisError`'s, so answering it is an **engine** question and goes through the test in `01-ontology.md` like any other |
...
saying two lines agree      the row's OLDEST item, named by seven results, and the answer is
                            assembled in `frontier/docs/MERGING.md` rather than restated
                            here. In one line: two records merge when one journal extends
                            the other, and the union of their intentions is FREE — what is
                            refused is the knowledge, which is the inversion of what the
                            item assumed. A candidate is reportable with nothing built, and
                            it is a candidate about knowledge and an instruction about
                            intention. What cannot be reported is that a merge happened
                            at all

a refusal that says which
side is missing (13's
Request 1), and a merge
that says what was
takeable (15)               ANSWERED TOGETHER, which is what the queue said they had to be.
                            `UnknownEntry` keeps the fault it always described correctly — a
                            coordinate moved in the right journal — and a true coordinate in
                            somebody else's journal now says so, discriminated by the
                            witness, which is where 13 said the other half was. `Diverged`
                            now carries what the two journals hold in common, symmetric
                            because converge is asymmetric in a way the question is not.
                            Guarded by the two experiments that measured the defects, whose
                            suites went red on the repair and are repaired in place
